<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
<%@ page errorPage="hata.jsp" %>
<%@ page import="java.sql.*"%>
<%@ page import="java.io.*" %>

<!DOCTYPE html>
<html lang="tr">
<head>
    <title>Ufak Proje</title>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="css/style.css">
    <!--[if lt IE 9]><script src="http://html5shiv.googlecode.com/svn/trunk/html5.js"></script><![endif]-->
</head>

<%
    String DRIVER = "com.postgresql.jdbc.Driver";
    Class.forName(DRIVER).newInstance();

    String connectionURL = "jdbc:postgresql://localhost:3306/ddddddd";
    String connectionUser = "dddd";
    String connectionPassword = "dfaşsdf";

    conn =

%>

<body>
<header class="center clearfix" id="navtop"> <a href="index.jsp" class="logo fleft"><img src="img/logo.png" alt="" /></a>
    <nav class="fright">
        <ul>
            <li><a href="index.jsp" class="navactive">Anasayfa</a></li>
            <li><a href="about.jsp">Hakkımızda</a></li>
        </ul>
        <ul>
            <li><a href="works.jsp">Çalışmalar</a></li>
            <li><a href="services.jsp">Hizmetler</a></li>
        </ul>
        <ul>
            <li><a href="blog.jsp">Blog</a></li>
            <li><a href="contact.jsp">İletişim</a></li>
        </ul>
    </nav>
</header>
<div class="main center">
    <section class="part clearfix">
        <header class="title clearfix">
            <p class="fleft">Anasayfa</p>
            <a href="about.jsp" class="arrow more fright">see more infos</a> </header>
        <div class="column2 mright">
            <h1>Web design <br>
                Web Development <br>
                Graphic Design</h1>
            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Morbi commodo, ipsum sed pharetra gravida, orci magna rhoncus neque, id pulvinar odio lorem non turpis. Nullam sit amet enim. Suspendisse id velit vitae ligula volutpat condimentum. Aliquam erat volutpat. Sed quis velit. </p>
            <p>Vivamus pharetra posuere sapien. Nam consectetuer. Sed aliquam, nunc eget euismod ullamcorper, lectus nunc ullamcorper orci, fermentum bibendum enim nibh eget ipsum. </p>
        </div>
        <div id="slides" class="slider column2">
            <div class="slides_container">
                <div class="slide">
                    <figure> <img src="img/4.jpg" alt="">
                        <figcaption>
                            <div>
                                <h5>Caption 1</h5>
                                <p>Lorem ipsum</p>
                            </div>
                        </figcaption>
                    </figure>
                </div>
                <div class="slide">
                    <figure> <img src="img/5.jpg" alt="">
                        <figcaption>
                            <div>
                                <h5>Caption 2</h5>
                                <p>Lorem ipsum dolor set amet, lorem, <br>
                                    <a href="#">link text</a></p>
                            </div>
                        </figcaption>
                    </figure>
                </div>
                <div class="slide">
                    <figure> <img src="img/6.jpg" alt="">
                        <figcaption>
                            <div>
                                <h5>Caption 3</h5>
                                <p>Lorem ipsum dolor set amet, lorem, variable length.</p>
                            </div>
                        </figcaption>
                    </figure>
                </div>
                <div class="slide">
                    <figure> <img src="img/9.jpg" alt="">
                        <figcaption>
                            <div>
                                <h5>Caption 4</h5>
                                <p>Lorem ipsum</p>
                            </div>
                        </figcaption>
                    </figure>
                </div>
            </div>
        </div>
    </section>
    <section class="part clearfix">
        <header class="title clearfix">
            <p class="fleft">Services</p>
            <a href="services.jsp" class="arrow more fright">see more services</a> </header>
        <article class="column3 mright services">
            <p class="number">01</p>
            <div class="sepmini"></div>
            <h5>Web design</h5>
            <div class="sepmini"></div>
            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Morbi commodo, ipsum sed pharetra gravida, orci magna rhoncus neque, id pulvinar odio lorem non turpis. Nullam sit amet enim.</p>
        </article>
        <article class="column3 mright services">
            <p class="number">02</p>
            <div class="sepmini"></div>
            <h5>Web development</h5>
            <div class="sepmini"></div>
            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Morbi commodo, ipsum sed pharetra gravida, orci magna rhoncus neque, id pulvinar odio lorem non turpis. Nullam sit amet enim.</p>
        </article>
        <article class="column3 services">
            <p class="number">03</p>
            <div class="sepmini"></div>
            <h5>Graphic design</h5>
            <div class="sepmini"></div>
            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Morbi commodo, ipsum sed pharetra gravida, orci magna rhoncus neque, id pulvinar odio lorem non turpis. Nullam sit amet enim.</p>
        </article>
    </section>
    <section class="part clearfix">
        <header class="title clearfix">
            <p class="fleft">Works</p>
            <a href="works.jsp" class="arrow more fright">see more works</a> </header>
        <figure class="work"> <a href="#"> <img src="img/10.jpg" alt=""> <span class="zoom"></span> </a>
            <figcaption> <a href="#" class="arrow">Projet 1</a>
                <p>Lorem ipsum dolor set amet</p>
            </figcaption>
        </figure>
        <figure class="work"> <a href="#"> <img src="img/11.jpg" alt=""> <span class="zoom"></span> </a>
            <figcaption> <a href="#" class="arrow">Projet 1</a>
                <p>Lorem ipsum dolor set amet</p>
            </figcaption>
        </figure>
        <figure class="work"> <a href="#"> <img src="img/12.jpg" alt=""> <span class="zoom"></span> </a>
            <figcaption> <a href="#" class="arrow">Projet 1</a>
                <p>Lorem ipsum dolor set amet</p>
            </figcaption>
        </figure>
        <figure class="work"> <a href="#"> <img src="img/13.jpg" alt=""> <span class="zoom"></span> </a>
            <figcaption> <a href="#" class="arrow">Projet 1</a>
                <p>Lorem ipsum dolor set amet</p>
            </figcaption>
        </figure>
    </section>
</div>
<footer class="center part clearfix">
    <ul class="social column3 mright">
        <li><a href="#">RSS</a></li>
        <li><a href="#">Facebook</a></li>
        <li><a href="#">Twitter</a></li>
        <li><a href="#">Google+</a></li>
        <li><a href="#">Flickr</a></li>
    </ul>
    <div class="up column3 mright"> <a href="#navtop" class="ir">Go up</a> </div>
    <nav class="column3">
        <ul>
            <li><a href="index.jsp">Home</a></li>
            <li><a href="about.jsp">About</a></li>
            <li><a href="works.jsp">Works</a></li>
            <li><a href="services.jsp">Services</a></li>
            <li><a href="blog.jsp">Blog</a></li>
            <li><a href="contact.jsp">Contact</a></li>
        </ul>
    </nav>
</footer>
<script src="http://code.jquery.com/jquery.min.js"></script>
<script>window.jQuery || document.write('<script src="js/jquery-1.5.1.min.js"><\/script>')</script>
<script src="js/slides.min.jquery.js"></script>
<script src="js/scripts.js"></script>
<!--[if (gte IE 6)&(lte IE 8)]>
<script src="js/selectivizr.js"></script>
<![endif]-->
</body>
</html>